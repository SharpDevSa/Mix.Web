...
using CheckboxSettings = DotOrg.Web.Ozi.PagesSettings.Lists.CheckboxSettings;
using CurrencySettings = DotOrg.Web.Ozi.PagesSettings.Lists.CurrencySettings;
using DateSettings = DotOrg.Web.Ozi.PagesSettings.Lists.DateSettings;
using ImageSettings = DotOrg.Web.Ozi.PagesSettings.Lists.ImageSettings;
using StringSettings = DotOrg.Web.Ozi.PagesSettings.Lists.StringSettings;
...
namespace DotOrg.Web.Ozi.PagesSettings
{
	public class Settings
	{
		...
		public FormSettings FormSettings
		{
			get
			{
				if (FormSettingsInternal == null)
				{
					...
					{
						Roles = formSettings.GetString("roles"),
		...
		{
			string path = System.Web.HttpContext.Current.Server.MapPath(Path.Combine(WebConfig.SettingsFolderPath, entityTypeName + ".xml"));
			...
			FormFieldsParsers = new Dictionary<XName, Func<XElement, FieldSettings>>
				{
					...
					{"code", ParseFormCode},
					{"color", ParseFormColor},
					{"address", ParseFormAddress},
					{"hr", ParseFormHr}
				};
		}
		...
		private static FieldSettings ParseFormAddress(XElement element)
		{
			return new AddressSettings
				{
					Longitude = element.GetString("lng"),
					Latitude = element.GetString("ltd")
				};
		}

		private static FieldSettings ParseFormColor(XElement element)
		{
			return new Forms.ColorSettings();
		}
        ...
        private static FieldSettings ParseFormTime(XElement element)
        {
            return new Forms.TimeSettings();
        }
		...
		private static FieldSettings ParseFormCollection(XElement element)
		{
			var result = new CollectionSettings
			{
				...
				AllowModify = element.GetBoolean("allowModify", true),
		...
		private static FieldSettings ParseFormFile(XElement element)
		{
			...
			{
				PathToSave = element.GetString("path", WebConfig.BasePathForImages),
				Type = element.GetString("type", ""),
				ShowTitle = element.GetBoolean("showTitle"),
				ShowClass = element.GetBoolean("showClass"),
		...
		{
			field.Roles = element.GetString("roles");
		...
		private static List<ListActionSettings> ParseListActions(XElement element)
		{
			...
			return listActions.Elements().Select(listAction =>
			{
				...
				{
					case "sort":
						return new SortActionSettings { Roles = listAction.GetString("roles") };
					case "visibility":
						return new VisibilityActionSettings { Roles = listAction.GetString("roles") };
					case "edit":
						return new ListActionSettings { Control = "edit", Roles = listAction.GetString("roles"), IgnoreRoles = true };
					case "delete":
						return new ListActionSettings { Control = "delete", Roles = listAction.GetString("roles") };
				}
				return null;
			...
			return cols.Elements().Select(field =>
			{
				...
				switch (field.Name.LocalName)
				{
					...
					case "checkbox":
						...
						break;
					case "color":
						column = new Lists.ColorSettings();
						break;
					case "filter":
				...
				column.Roles = field.GetString("roles");
				...
				column.Sortable = field.GetBoolean("sortable", true);
				return column;
			}).ToList();
		}
		...
		private static List<GlobalActionSettings> ParseGlobalActions(XElement element)
		{
			...
			return globalActions.Elements().Select(field =>
			{
				...
				{
					return new CreateGlobalActionSettings { Roles = field.GetString("roles") };
				}
				if (field.Name.LocalName == "localize")
				{
					return new LocalizeGlobalActionSettings { Roles = field.GetString("roles") };
				}
				if (field.Name.LocalName == "custom")
				{
					return new GlobalActionSettings { Control = field.GetString("control"), Action = field.GetString("action"), Text = field.GetString("text"), Roles = field.GetString("roles") };
				}
				return null;
			}).ToList();
		}
