	...
	public class OziModelBinder : DefaultModelBinder
	{
		...
		protected override object CreateModel(ControllerContext controllerContext, ModelBindingContext bindingContext, Type modelType)
		{
			...
			if (isEntityCollection)
			{
				...
				//db.TryGetObjectByKey(new EntityKey(Libs.TypeHelpers.GetEntitySetName(typeToCreate, db), "Id", id.ConvertTo(typeof(int))), out result);
				result = db.Set(typeToCreate).Find(id.ConvertTo(typeof(int)));
			...
			if (result == null)
			{
				...
				if (isEntityCollection)
				{
					db.Set(typeToCreate).Add(result);
				}
			}
			return result;
		}
